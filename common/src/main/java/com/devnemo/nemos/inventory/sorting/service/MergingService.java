...
public class MergingService {
    ...
    public void mergeAllItems(AbstractContainerMenu menu, List<SlotItem> sortedSlotItems, int containerId) {
        var groupedItemByCompoundTagMap = sortedSlotItems.stream()
                .filter(slotItem -> slotItem.itemStack().getTag() != null)
                .collect(groupingBy(slotItem -> slotItem.itemStack().getTag()));
        var groupedItemByItemNameMap = sortedSlotItems.stream()
                .filter(slotItem -> slotItem.itemStack().getTag() == null)
                .collect(
                        groupingBy(slotItem -> {
                            var itemStack = slotItem.itemStack();

                            return itemStack.getItem().getName(itemStack).getString();
                        })
                );

        groupedItemByItemNameMap.forEach((key, slotItems) -> mergeItems(menu, slotItems, containerId));
        groupedItemByCompoundTagMap.forEach((key, slotItems) -> mergeItems(menu, slotItems, containerId));
    }
